import { Chat, Message, MessageCandidate } from "@shared/db_types";
import { CardBundle, Result } from "@shared/types";
...
async function createChat(cardID: number): Promise<Result<RunResult, Error>> {
  try {
    const query = `
      INSERT INTO chats (card_id)
      VALUES (?);`;
    const params = [cardID];
